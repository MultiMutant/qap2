...
function createVideo(src, width, controls) {
  src = src.trim();
  let tagVideo = '<video src="${src}"';

  if (Number.isInteger(width)) {
    tagVideo += ' width="%{width}"';
  }
...
function parseDateString(value) {
  if (typeof value !== 'string' || value === null) {
    throw new Error("Input is invalid: Date string must be a valid string in the format: YYYY-MM-DD");
  }
  ...
  if (yearStr.length !== 4) {
    throw new Error("Year format is invalid: Year must be 4 digits.");
}

if (monthStr.length !== 2) {
    throw new Error("Month format is invalid: Month must be 2 digits.");
}

if (dayStr.length !== 2) {
    throw new Error("Day format is invalid: Day must be 2 digits.");
}
...
function toDateString(value) {
  ...
  if (!(value instanceof Date) || isNaN(value)) {
    throw new Error("Date format is invalid: The value must be a valid Date object.");
  }


  const year = value.getFullYear();
  const month = String(value.getMonth() + 1).padStart(2, '0'); // months are weird, 0 is Jan, 1 is Feb ect
...
function formatCoords(...values) {
  ...
  values.forEach(value => {
    try {
      const [lat, long] = normalizeCoord(value);
...
function generateLicenseLink(licenseCode, targetBlank = false) {
  ...
  let targetAttribute = targetBlank ? ' target="_blank"' : '';

  return `<a href="${licenseLink}"${targetAttribute}>${licenseText}</a>`;
}
...
function pureBool(value) {
  ...
  const falseValues = [
    'no', 'n', 'non', 'f', 'false', 'faux', '0', '-1'
  ];

  // checkin if its yay or nay
...
function buildUrl(query, order, count, license) {
  ...
  if (typeof count !== 'number' || count < 1 || count > 50) {
    throw new Error("Invalid number. Must be between 1 and 50.");
  }

  const validLicenses = ['none', 'any', 'cc-by', 'cc-by-nc', 'cc-by-sa', 'cc-by-nd', 'cc-by-nc-sa', 'cc-by-nc-nd'];
  if (!validLicenses.includes(license)) {
    throw new Error("Invalid license. Must be one of 'none', 'any', 'cc-by', 'cc-by-nc', 'cc-by-sa', 'cc-by-nd', 'cc-by-nc-sa', 'cc-by-nc-nd'.");
  }
